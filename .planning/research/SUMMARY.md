# Sintesi Ricerca di Progetto — SemBridge
...
**Should-have (i 3-5 differenziatori principali di SemBridge):**
...
4. **Worker integration come route type (DIFF-04)** — `type: 'worker', task: 'generateReport'`. Comlink offre RPC, ma SemBridge integra worker col canonical model + retry + cache + debug pipeline.
...
- Adapter framework UI (`@sembridge/react`, `@sembridge/vue` con `useSubscribe`/`usePublish`)
- IndexedDB persistence (PRD §20.3 lascia opzionale; iniziare con `@sembridge/cache-idb` package separato in V1.x)
