...
- **Project:** GlueZero — libreria JavaScript browser-side TypeScript-first (ESM) per pub/sub, routing, canonical model, gateway server unico, worker runtime, developer tooling
...
- DOC-02 `packages/gluezero/README.md` italiano 452 LOC con scenario meteo F1+F2+F3+F4+F5+F6 end-to-end + chain composition outermost devtools→cache→worker→realtime→router→mapper→broker + Q&A 8
- DOC `packages/gluezero/EXAMPLES.md` italiano 519 LOC con 10 esempi consolidati cross-feature + scenario meteo full chain
