...
- [x] **TOOL-02**: Metrics: eventi/sec, eventi scartati, errori per categoria, tempi medi route HTTP/worker, cache hit ratio, subscriber per topic, backlog *(PRD §25.5)* — Complete (plan 06-06): MetricsCollector counters/gauges/histograms simil-OpenMetrics + naming `sembridge.<package>.<metric>{<labels>}` Prometheus + reservoir Algorithm R Vitter 1985 (D-163/D-164/D-165) + cardinality cap 100 + audit (D-166)
...
- [x] **DOC-02**: Guida integrazione plugin *(PRD §41.4)* — Complete (plan 06-09b): `packages/sembridge/README.md` italiano ~452 LOC con scenario meteo F1+F2+F3+F4+F5+F6 end-to-end + chain composition + Q&A 8 domande
...
- [x] **DOC-05**: Esempi end-to-end (incluso scenario meteo PRD §29) *(PRD §41.8)* — Complete (plan 06-09b): `packages/sembridge/EXAMPLES.md` italiano ~519 LOC consolidato F1+F2+F3+F4+F5+F6 + scenario cross-feature integrato — closes PRD §39 #10 (TOOL-05) in `packages/devtools/README.md` Sezione 6 con 7 Q&A enumerate (Q1-Q7)
...
| TOOL-02 | Phase 6 | Complete (plan 06-06) | MetricsCollector counters/gauges/histograms simil-OpenMetrics + naming `sembridge.<package>.<metric>{<labels>}` Prometheus + reservoir Algorithm R Vitter 1985 (D-163/D-164/D-165) + cardinality cap 100 + audit (D-166) |
...
| DOC-02 | Phase 6 | Complete (plan 06-09b) | Guida integrazione plugin v1.0 — `packages/sembridge/README.md` italiano ~452 LOC con scenario meteo F1+F2+F3+F4+F5+F6 end-to-end + chain composition outermost devtools→cache→worker→realtime→router→mapper→broker + features opt-out + Q&A 8 domande |
