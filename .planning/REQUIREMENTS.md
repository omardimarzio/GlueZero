...
- [x] **CACHE-02**: TTL configurabile e invalidazione manuale/automatica *(PRD §20.2)* — Complete (plan 06-02 + 06-08a): TTL ortogonale a LRU + invalidate API (string/RegExp/{prefix}) + cascade unregisterPlugin invalidate by ownerId (LIFE-02 ext F6 D-126)
- [x] **CACHE-03**: Metadata di consegna distingue origine `cache` vs `remote` *(PRD §20.2, §20.4)* — Complete (plan 06-03): metadata.origin popolato dal CacheHandlerF6 3-strategy + cache-then-network ordering microtask (D-156 RESEARCH §15.6)
- [x] **TOOL-01**: Event Inspector completo: topic, publisher, timestamp, payload originale/canonico, subscriber raggiunti, route attivate, esito, errori, tempi *(PRD §25.1)* — Complete (plan 06-05 + 06-08b): EventInspector ring buffer 500 + RouteInspector capture step 9+10 + tap registry chain D-159 + getBuffer deep-clone D-162
- [x] **TOOL-02**: Metrics: eventi/sec, eventi scartati, errori per categoria, tempi medi route HTTP/worker, cache hit ratio, subscriber per topic, backlog *(PRD §25.5)* — Complete (plan 06-06): MetricsCollector counters/gauges/histograms simil-OpenMetrics + naming `gluezero.<package>.<metric>{<labels>}` Prometheus + reservoir Algorithm R Vitter 1985 (D-163/D-164/D-165) + cardinality cap 100 + audit (D-166)
...
- [x] **TOOL-04**: `pauseTopic(topic)` / `resumeTopic(topic)` / `flushQueue(topic?)` *(PRD §16.3)* — Complete (plan 06-07 + 06-08b): PauseController D-168 pauseTopic block FIFO + D-169 flushQueue drop + system.queue.flushed audit + D-170 cap drop-oldest + critical bypass (Pitfall 4.C carryover F3 D-75 + F5 D-130)
- [x] **TOOL-05**: Format delle metriche documentato esplicitamente *(PRD §39 — open issue da chiudere)* — Complete (plan 06-06 + 06-09b DOC-05) — **Closes PRD §39 #10** ✅ 2026-05-05: schema `{ counters, gauges, histograms }` simil-OpenMetrics naming `gluezero.<package>.<metric>{<labels>}` dot.case Prometheus + cumulative-only counters (D-164) + helper getMetricsDelta + reservoir Algorithm R Vitter 1985 (D-165) + cardinality cap 100 distinct combinations + audit (D-166). DOC-05 `packages/devtools/README.md` italiano 11 sezioni — Sezione 6 "MetricsCollector — closes PRD §39 #10 (TOOL-05)" con tabella naming convention + 7 Q&A enumerate (Q1 dot.case rationale, Q2 getMetricsDelta, Q3 reservoir vs t-digest, Q4 cardinality overflow, Q5 Prometheus/OTel exporter, Q6 metriche standard, Q7 custom metric V1.x) + anti-pattern cardinality explosion
...
- [x] **DOC-02**: Guida integrazione plugin *(PRD §41.4)* — Complete (plan 06-09b): `packages/gluezero/README.md` italiano ~452 LOC con scenario meteo F1+F2+F3+F4+F5+F6 end-to-end + chain composition + Q&A 8 domande
...
- [x] **DOC-05**: Esempi end-to-end (incluso scenario meteo PRD §29) *(PRD §41.8)* — Complete (plan 06-09b): `packages/gluezero/EXAMPLES.md` italiano ~519 LOC consolidato F1+F2+F3+F4+F5+F6 + scenario cross-feature integrato — closes PRD §39 #10 (TOOL-05) in `packages/devtools/README.md` Sezione 6 con 7 Q&A enumerate (Q1-Q7)
- [x] **DOC-06**: Documentazione debug tooling *(PRD §41.9)* — Complete (plan 06-09b): `packages/devtools/README.md` italiano ~368 LOC 11 sezioni con anti-pattern cardinality explosion + 7 Q&A enumerate (WARNING-2 fix) + structuredClone perf caveat + PRD §39 #10 closure
...
| Sostituzione di EventEmitter generici (mitt, eventemitter3) | GlueZero è middleware con canonical model + gateway, non un emitter |
...
| CORE-10 | Phase 1 | Done (plan 01-04) | `createConsoleLogger(level)` + `silentLogger` in `core/logger.ts` — 6 livelli, namespace `[gluezero]`, D-12 mapping completo, 11/11 test passing |
...
| Requirement | Phase | Status | Note |
|-------------|-------|--------|------|
| CACHE-01 | Phase 6 | Complete (plan 06-02 + 06-03 + 06-08a) | MemoryCacheAdapter LRU bounded `maxEntries=1000` (D-158) + cacheKey D-155 riuso F3 D-74 + override route-level. IDB rinviato a V1.x |
| CACHE-02 | Phase 6 | Complete (plan 06-02 + 06-08a) | TTL ortogonale a LRU + invalidate API (string/RegExp/{prefix}) + cascade unregisterPlugin invalidate by ownerId (LIFE-02 ext F6 D-126) |
| CACHE-03 | Phase 6 | Complete (plan 06-03) | metadata.origin `cache` vs `remote` popolato dal CacheHandlerF6 3-strategy + cache-then-network ordering microtask (D-156 RESEARCH §15.6) |
| TOOL-01 | Phase 6 | Complete (plan 06-05 + 06-08b) | EventInspector ring buffer 500 (D-167) + RouteInspector capture step 9+10 + tap registry chain D-159 + getBuffer deep-clone D-162 |
| TOOL-02 | Phase 6 | Complete (plan 06-06) | MetricsCollector counters/gauges/histograms simil-OpenMetrics + naming `gluezero.<package>.<metric>{<labels>}` Prometheus + reservoir Algorithm R Vitter 1985 (D-163/D-164/D-165) + cardinality cap 100 + audit (D-166) |
| TOOL-03 | Phase 6 | Complete (plan 06-08b) | DevtoolsBroker.enableDebug/disableDebug toggle live-mode (D-160 + NODE_ENV detection) + getDebugSnapshot deep-clone via structuredClone (D-162) |
| TOOL-04 | Phase 6 | Complete (plan 06-07 + 06-08b) | PauseController D-168 pauseTopic block FIFO + D-169 flushQueue drop + `system.queue.flushed` audit + D-170 cap drop-oldest + critical bypass (Pitfall 4.C carryover F3 D-75 + F5 D-130) |
| TOOL-05 | Phase 6 | Complete (plan 06-06 + 06-09b DOC-05) — **Closes PRD §39 #10** ✅ 2026-05-05 | schema `{ counters, gauges, histograms }` simil-OpenMetrics naming `gluezero.<package>.<metric>` dot.case Prometheus + cumulative-only (D-164) + helper getMetricsDelta + reservoir Algorithm R Vitter 1985 (D-165) + cardinality cap 100 + audit (D-166). DOC-05 `packages/devtools/README.md` italiano Sezione 6 con tabella naming convention + 7 Q&A enumerate (Q1 dot.case rationale, Q2 getMetricsDelta, Q3 reservoir vs t-digest, Q4 cardinality overflow, Q5 Prometheus/OTel exporter, Q6 metriche standard, Q7 custom metric V1.x) + anti-pattern cardinality explosion |
...
| ERR-02 | Phase 2 | Complete (F6 ext closed plan 06-03 + 06-06 + 06-07 + 06-08b) | F2: `mapping.error`, F3: `<topic>.failed`+`network.error`, F4: `system.realtime.connected/disconnected/reconnecting/failed`, F5: `worker.error` topic ext + sanitized payload, **F6: `system.cache.scope-missing` (D-157) + `system.queue.flushed` (D-169) + `system.queue.overflow` (D-170) + `system.metrics.cardinalityoverflow` (D-166) + sanitized error D-80 carryover in CacheHandlerF6 (`cache.network.failed`, `cache.strategy.unknown` con shape `{ code, category, message, routeId, topic, eventId }` — NO originalError/stack/cause)** |
...
| PIPE-01 | Phase 1 (skeleton) | Complete (F6 closed plan 06-08b) | Estesa da F2 (step 4-6, 11-12), F3 (step 7-10), **F6 step 14 reale attivato via DevtoolsBroker post `inner.publish` (D-161 lifecycle events `event.observed` + tap chain MultiplexTap D-159) — pipeline §28 14 step ora completa end-to-end** |
...
| LIFE-02 | Phase 1 | Done (plan 01-08) + F6 ext closed (plan 06-08a + 06-08b) | **Closes PRD §39 #7**: cascade D-26 deterministico in `unregister(id)` — `bus.unsubscribeByOwner` → `abortController.abort()` → `onUnmount` → `onDestroy`. F3 cascade route via `RouterEngine.unregisterByOwner`; F4 cascade realtime via `RealtimeChannelManager.disconnectByOwner` (D-112); F5 cascade worker via `WorkerRegistry.unregisterByOwner` + `WorkerPool.terminateByOwner` 3-step idempotente; **F6 cascade cache via `adapter.invalidate({ prefix: '<ownerId>::' })` 2-step idempotente (inner.unregisterPlugin + cache invalidate) — convention D-156 scope hybrid prefix isolation + try/catch isolato per ogni step (closed plan 06-08a + 06-08b)**. |
...
| TEST-01 | Phase 1 (subset) | Done subset + F4 ext + F5 ext + **F6 ext closed (plan 06-02..06-08b)** | PipelineHarness fixture + 8 integration test F1 + 14 integration test 3-tier F4 + 121 worker test F5 + 6 browser smoke Tier-3 Playwright Chromium reale. **F6 ext: 100+ unit test Tier-1 jsdom su packages/cache/src/ (memory-cache-adapter + stable-hash + cache-handler + composite-handler + cache-broker + public-factory) + packages/devtools/src/ (multiplex-tap + tap-registry + event-inspector + route-inspector + reservoir-sampling + cardinality-cap + metrics-collector + pause-controller + devtools-broker + public-factory) + packages/gluezero/src/ (sem-bridge) — coverage v8 cache 100/94.21/100/100, devtools 96.44/89.28/94.36/96.98, sembridge 100/100/100/100 sopra hard floor target ≥90/80/90/90**. |
| TEST-02 | Phase 2 (subset) | Complete (F6 ext closed plan 06-08a + 06-08b) | F3 (server), F4 (reconnect via `__integration__/sse-reconnect.test.ts` + `auto-fallback.test.ts`), F5 (worker) 8 integration Tier-1 + 6 browser Tier-3, **F6 (cache + devtools) closed plan 06-08a + 06-08b: 4 integration test cache 3-tier (Tier-1 jsdom: cache-then-network ordering microtask + cascade invalidate by ownerId D-126 + scope-missing audit D-157 + LRU eviction) + 6 integration test devtools+sembridge 3-tier (createGlueZero chain F1+F2+F3+F4+F5+F6 + getDebugSnapshot deep-clone D-162 + pauseTopic FIFO D-168 + flushQueue audit D-169 + critical bypass D-170 + tap chain MultiplexTap D-159)**. |
...
| DOC-02 | Phase 6 | Complete (plan 06-09b) | Guida integrazione plugin v1.0 — `packages/gluezero/README.md` italiano ~452 LOC con scenario meteo F1+F2+F3+F4+F5+F6 end-to-end + chain composition outermost devtools→cache→worker→realtime→router→mapper→broker + features opt-out + Q&A 8 domande |
...
| DOC-05 | Phase 6 | Complete (plan 06-09b) | `packages/gluezero/EXAMPLES.md` italiano ~519 LOC con 10 esempi end-to-end consolidati F1+F2+F3+F4+F5+F6 (Hello World pub/sub + canonical mapping + HTTP route retry/timeout + SSE realtime + worker offload + cache-then-network + Inspector dashboard + pauseTopic admin + multi-tenant scope D-156 + cross-feature integrato F1+F2+F3+F4+F5+F6 scenario meteo full chain). Carryover F5 worker section delivered. |
| DOC-06 | Phase 6 | Complete (plan 06-09b) | `packages/devtools/README.md` italiano ~368 LOC 11 sezioni con anti-pattern cardinality explosion + 7 Q&A enumerate (WARNING-2 fix Q1-Q7) + structuredClone perf caveat + PRD §39 #10 closure (TOOL-05 metrics format) — schema simil-OpenMetrics naming `gluezero.<package>.<metric>{<labels>}` Prometheus + reservoir Algorithm R Vitter 1985 |
...
- **Closed:** #7 (LIFE-02 — Phase 1 plan 01-08); #1 (MAP-17 — Phase 2 plan 02-04); #3 (VAL-08 — Phase 2 plan 02-07); #4 (VAL-09 — Phase 2 plan 02-05); #5 (ROUTE-16 — Phase 3 plan 03-12); #6 (ROUTE-15 — Phase 3 plan 03-05); #8 (ROUTE-09 — Phase 3 plan 03-09); #9 (RT-07 — Phase 4 plan 04-09); #11 (WK-07 — Phase 5 plan 05-07); **#10 (TOOL-05 — Phase 6 plan 06-06 + 06-09b DOC-05: schema `{ counters, gauges, histograms }` simil-OpenMetrics + naming `gluezero.<package>.<metric>{<labels>}` dot.case Prometheus + cumulative-only counters D-164 + helper getMetricsDelta + reservoir Algorithm R Vitter 1985 D-165 + cardinality cap 100 distinct combinations + audit D-166. DOC-05 `packages/devtools/README.md` italiano Sezione 6 "MetricsCollector — closes PRD §39 #10 (TOOL-05)" con tabella naming convention + 7 Q&A enumerate Q1-Q7 + anti-pattern cardinality explosion)** ✅ CLOSED 2026-05-05
- **Open:** #2 (cross-fase pipeline ordering, F1+F2+F3+F6) — *deferred V1.x: pipeline ordering è documentato implicitamente in pipeline §28 14 step e nei DOC F1-F6, ma una sezione dedicata "Pipeline ordering canonical" cross-fase è opt-in V1.x quando emergeranno consumer cross-fase reali in produzione*

---
*Requirements defined: 2026-04-28*
*Last updated: 2026-05-05 after Phase 6 closure (plan 06-09b): CACHE-01..03 + TOOL-01..05 + DOC-02/05/06 all Complete + ERR-02 ext F6 + LIFE-02 ext F6 + PIPE-01 ext F6 + TEST-01/02 ext F6 + chiusura PRD §39 #10 (TOOL-05 metrics format documentato). **MILESTONE v1.0 ✅ CHIUSA** — 6/6 fasi complete + 10/11 open issues PRD §39 closed (#2 cross-fase pipeline ordering deferred V1.x).*
