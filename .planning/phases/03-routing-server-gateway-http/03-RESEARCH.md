...
Fase 3 consegna due package gemelli (`@sembridge/routing` + `@sembridge/gateway`) che insieme estendono il `MapperBroker` di F2 con un **Routing Engine dichiarativo** (4 tipi route — `local`/`http`/`cache`/`composite`) e un **HTTP Gateway** centralizzato che applica policy uniformi a ogni fetch (timeout, retry 4xx/5xx differenziato, dedupe, backpressure, auth Bearer + refresh, idempotency, URL allowlist, cancellazione). La fase chiude 4 open issue PRD §39 (#5 ROUTE-16, #6 ROUTE-15, #7 LIFE-02 ext, #8 ROUTE-09) e introduce 4 nuovi step della pipeline §28 (step 7-full backpressure, step 8 route resolved, step 9 route executed, step 10 outcome collected).
...
**Primary recommendation:** Spezzare F3 in **12 plan** organizzati in **6 wave** (4 wave fortemente parallelizzabili per file ownership disgiunta), con `@sembridge/gateway` esposto via **subpath exports** (`@sembridge/gateway/http` per F3, `@sembridge/gateway/sse-ws` riservato a F4). Test integration scenario meteo HTTP via `msw` 2.x in mode Node (già installato) con handler-set domain-grouped + per-test overrides. Performance budget: routing < 5 KB gz, gateway/http < 6 KB gz, overhead route HTTP < 50ms (escluso fetch network), zero overhead per topic `'local'` rispetto a F2.
...
- **D-62:** `RoutingConfig` opzionale in `BrokerConfig` (sezione `routes?` + `gateway?`). Pattern non-breaking via TS declaration merging in `@sembridge/routing/src/augment.ts` e `@sembridge/gateway/src/augment.ts` (replica F2 D-56).
...
- **D-85:** **`PipelineStep` extension F3** via TS declaration merging in `@sembridge/routing/src/augment.ts` (replicato pattern F2 D-49). `safeTapStep` di F1 riusato per i tap.
...
- **D-92:** **Coverage v8 F3** ≥ 90% sui file `@sembridge/routing/` e `@sembridge/gateway/http/`.

**K. Type System**
- **D-93:** Type re-export da `@sembridge/routing` e `@sembridge/gateway` a `@sembridge/core` via TS declaration merging (replicato F2 D-56).
...
- **`@sembridge/gateway/http` vs `@sembridge/gateway`** — questo research **raccomanda subpath exports** (vedi sezione **Subpath Exports Recommendation**)
...
| ROUTE-06 | Server Gateway centralizza fetch | `@sembridge/gateway/http` con `HttpGateway` class. Tutte route HTTP DEVONO passare per il gateway (no fetch diretto da plugin) |
...
|------------|-------------|----------------|-----------|
| Route resolution (topic → RouteDefinition) | Browser / Client (`@sembridge/routing`) | — | Pre-compiled dispatch table (D-64) lives in browser memory; resolution è O(1) hot-path lookup |
| Route execution dispatch | Browser / Client (`@sembridge/routing`) | API / Backend (per route HTTP) | Executor decide il tipo (local/http/cache/composite) e delega; HTTP backend è fuori scope librario |
| Request build (queryMap/bodyMap canonical → server) | Browser / Client (`@sembridge/routing` invoca `@sembridge/mapper`) | — | Riuso MapperEngine F2 — D-96 |
| HTTP fetch + policy chain | Browser / Client (`@sembridge/gateway/http`) | API / Backend (server-side responsibility for retry/idempotency contract) | fetch nativo + Strategy Pattern; backend deve onorare `Idempotency-Key` |
| Response parse + canonical mapping | Browser / Client (`@sembridge/gateway/http` + `@sembridge/mapper`) | — | Default JSON parse + MapperEngine inverso server→canonical (D-97) |
| Response validation (VAL-05) | Browser / Client (`@sembridge/routing` invoca `valibotAdapter` di F2) | — | Riuso pipeline F2 — opt-in via `response: { canonical: 'id' }` |
| AbortController lifecycle | Browser / Client (`@sembridge/routing` + `@sembridge/gateway/http`) | — | Browser AbortSignal API; cascade da unsubscribe → fetch.signal |
| Auth header injection + refresh | Browser / Client (`@sembridge/gateway/http` AuthStrategy) | API / Backend (issues token via 401/refresh endpoint) | Browser legge token via callback config; refresh è browser-orchestrato |
| URL allowlist enforcement | Browser / Client (`@sembridge/gateway/http`) | — | Pre-fetch guard — runtime validation prevention contro client-side mistake |
| Idempotency token generation | Browser / Client (`@sembridge/gateway/http` + nanoid) | API / Backend (server deduplicates by key) | Client-generated `nanoid()`; precondizione documentata DOC-04: server-side dedup obbligatorio per garanzia E2E |
| Pipeline tap step 8/9/10 | Browser / Client (`@sembridge/routing`) | — | EventTap pre-instrumented in F1 — F3 emette no-op snapshot, F6 wirea Inspector reale |
...
| **`valibot`** | 1.3.1 | Response schema validation (riuso F2) | [VERIFIED: pnpm-lock.yaml] Già installato in F2. Riuso `valibotAdapter` di `@sembridge/mapper` |
| **`@sembridge/core`** | workspace:* | Broker base + EventTap | [VERIFIED: file packages/core/] F1 complete |
| **`@sembridge/mapper`** | workspace:* | MapperEngine + canonical schemas | [VERIFIED: file packages/mapper/] F2 complete |
...
pnpm add @sembridge/core@workspace:* @sembridge/mapper@workspace:* nanoid@5.1.9 valibot@1.3.1 --filter @sembridge/routing
pnpm add @sembridge/core@workspace:* @sembridge/mapper@workspace:* nanoid@5.1.9 --filter @sembridge/gateway
pnpm add -D tsup@8.5.1 vitest@4.1.5 typescript@6.0.3 jsdom@29.1.0 --filter @sembridge/routing
pnpm add -D tsup@8.5.1 vitest@4.1.5 typescript@6.0.3 jsdom@29.1.0 --filter @sembridge/gateway
```
...
**Decisione:** `@sembridge/gateway` USA **subpath exports** per separare HTTP (F3) e SSE/WS (F4):

- `@sembridge/gateway/http` — Phase 3 (questo phase)
- `@sembridge/gateway/sse-ws` — Phase 4
...
2. **Dependency boundary chiaro**: il `RouterBroker` di F3 può importare SOLO da `@sembridge/gateway/http` — cross-import F3→F4 sub-module impossibile per costruzione.
...
**Alternativa rejected:** package separati (`@sembridge/gateway-http` + `@sembridge/gateway-sse`) — overhead di 2 package.json + 2 build pipeline + 2 changeset entries; subpath exports è il sweet spot.
...
│   ├── http-handler.ts              # invoca @sembridge/gateway/http
...
import { TopicTrie } from '@sembridge/core/internal' // se esposto, altrimenti riuso via composition
...
**Verifica supporto:** target ES2022 di SemBridge (PKG-03) NON include `AbortSignal.any()` (ES2024). **Decisione: usare il fallback `combineSignals` polyfill di ~10 LOC sopra**. Documentare in DOC-04 che target ES2022 richiede questa helper interna. F4 quando target sale a ES2024 può rimuovere il polyfill.
...
- **fetch diretto da plugin senza gateway:** Viola ROUTE-06 + SEC-05 (no allowlist). Tutte route HTTP DEVONO passare per `@sembridge/gateway/http`.
...
|------|------|------|-------------------------|-------------|
| **1** | **03-01** | Bootstrap `@sembridge/routing` + `@sembridge/gateway` (tsup config, vitest config, tsconfig, deps wiring, size-limit, sideEffects array, README skeleton) | `packages/routing/{tsup.config,vitest.config,tsconfig,package.json,README.md,src/index.ts}` + idem gateway | sequenziale |
...
| A5 | Subpath exports `@sembridge/gateway/http` funziona con tsup multi-entry zero-config | Subpath Exports Recommendation | LOW — pattern consolidato (`@aws-sdk/*`, `@octokit/*`); tsup `entry: { ... }` documentato |
...
- **`prd.md`** — fonte autoritativa unica del progetto SemBridge (§17 Routing Engine, §18 Gateway Server, §22.3 Eventi standard di errore, §23 Retry/Timeout/Backpressure/Deduplica, §26 Sicurezza, §28 Pipeline ufficiale, §29 Scenario meteo, §39 #5/#6/#7/#8 open issues)
...
- **`packages/mapper/src/augment.ts`** — TS declaration merging pattern (replica per `@sembridge/routing/src/augment.ts`)
