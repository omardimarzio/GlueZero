...
requires:
  - phase: 02-canonical-model-mapper
    plan: 01
    provides: "@sembridge/mapper buildable + workspace link a @sembridge/core + size-limit budget 5 KB + coverage v8 abilitata"
provides:
  - "6 file packages/mapper/src/types/*.ts (5 modulo + 1 barrel) con superficie tipi pubblici F2 completa"
  - "Branded types `CanonicalSchemaId` + `TransformName` (Pitfall #12 — type confusion prevention) replicando pattern EventId di @sembridge/core"
...
tech-stack:
  added: []
  patterns:
    - "Branded type con `unique symbol` distinto per ogni id (CanonicalSchemaId vs TransformName) — replicato da `EventId` di @sembridge/core/types/broker-event.ts:54-61"
...
patterns-established:
  - "Pattern bootstrap tipi pubblici nuovo package F2+: copia esatta della shape `packages/core/src/types/index.ts` (riga 1-41) con JSDoc 1-liner per ogni export — riusabile per @sembridge/{routing, gateway, worker, cache, devtools} in F3-F6"
...
- Cross-package import `import type { BrokerLogger } from '@sembridge/core'` in `transform.ts` con `import type` esplicito (verbatimModuleSyntax)
...
- **`transform.ts`** (66 LOC) — Esporta:
  ...
  - Import: `import type { BrokerLogger } from '@sembridge/core'` (verbatimModuleSyntax)
...
- **`index.ts`** (61 LOC barrel) — `export type { ... }` di:
  ...
  - JSDoc 1-liner su ogni export (pattern @sembridge/core/types/index.ts)

## Verification

| Comando | Risultato |
|---------|-----------|
| `pnpm --filter @sembridge/mapper typecheck` (post Task 1) | Exit 0 (no errori TS, isolatedDeclarations enforcement OK) |
| `pnpm --filter @sembridge/mapper typecheck` (post Task 2) | Exit 0 |
| `pnpm --filter @sembridge/mapper test` | Exit 0 (passWithNoTests; nessun test runtime in questo plan, atteso — TDD inizia in 02-03) |
| `pnpm --filter @sembridge/mapper build` | Exit 0; `dist/index.js` 68 B + `dist/index.d.ts` 13 B (barrel principale ancora skeleton — coerente con plan 02-09 che attiverà il wiring) |
...
- ⏳ **Pending:** plan 02-12 (final gate F2) misurerà coverage v8 ≥ 90% sui file `@sembridge/mapper/`.
