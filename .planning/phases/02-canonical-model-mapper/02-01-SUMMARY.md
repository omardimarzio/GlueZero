...
requires:
  - phase: 01-core-essenziale
    provides: "@gluezero/core buildato + workspace pnpm + tsconfig.base.json + biome.json + size-limit + tsup/vitest setup pattern"
provides:
  - "Package @gluezero/mapper buildable + testable + typed-checked (placeholder runtime, dist artifact valido)"
  - "Workspace link @gluezero/core: workspace:* attivo per import cross-package F2 successivi"
...
tech-stack:
  ...
  patterns: ["sideEffects array per declaration merging file (./dist/augment.js)", "external @gluezero/core nel tsup (peer-like, monorepo workspace protocol)", "scripts/dependencies/devDependencies allineate F1 esattamente per consistency"]

key-files:
  created:
    - "packages/mapper/tsconfig.json (replica esatta core, eredita tsconfig.base.json)"
    - "packages/mapper/tsup.config.ts (replica core con external @gluezero/core)"
    - "packages/mapper/vitest.config.ts (name @gluezero/mapper + coverage v8 thresholds 90/85/90/90)"
...
key-decisions:
  ...
  - "tsup external: [/^node:/, '@gluezero/core'] (peer-like) — non bundla @gluezero/core dentro mapper, usa workspace protocol pnpm per il link runtime"
...
patterns-established:
  - "Pattern bootstrap nuovo package monorepo: replica esatta dei 4 file config @gluezero/core (package.json, tsup.config.ts, tsconfig.json, vitest.config.ts) con 3 diff puntuali (name, sideEffects, external) — riusabile per @gluezero/{gateway, routing, worker, cache, devtools} in F3-F6"
...
# Phase 2 Plan 01: Bootstrap @gluezero/mapper Summary

**Package @gluezero/mapper passa da placeholder F1 (18 LOC, src vuoto) a package buildable+testable+typed-checked, con workspace link a @gluezero/core, coverage v8 abilitata, e size-limit budget 5 KB gzip — pronto per popolamento moduli da plan 02-02 a 02-12.**
...
## Accomplishments
- 4 file config (`package.json`, `tsup.config.ts`, `tsconfig.json`, `vitest.config.ts`) per `@gluezero/mapper` allineati esattamente a `@gluezero/core` (replica D-15 frozen reference Phase 1)
- Workspace link `@gluezero/core: workspace:*` attivo (verificato da `pnpm install` exit 0)
...
1. **Task 1: Replica config @gluezero/core in @gluezero/mapper** — `b200948` (chore)
...
- `packages/mapper/tsup.config.ts` — replica core con 2 diff: `external: [/^node:/, '@gluezero/core']` (peer-like, evita doppio bundle); `banner` con name `@gluezero/mapper`
...
- `packages/mapper/package.json` — da placeholder 18 LOC a package buildable F2 (52 LOC):
  - `name: '@gluezero/mapper'`, `version: '0.0.0'`
  ...
  - `dependencies`: `@gluezero/core: workspace:*` + `valibot: 1.3.1`
...
- `package.json` (root) — aggiunge `@vitest/coverage-v8: 4.1.5` in `devDependencies` + entry size-limit per `@gluezero/mapper (gzip)` con limit 5 KB
- `pnpm-lock.yaml` — aggiornato con `@vitest/coverage-v8` (+ deps transitive `@bcoe/v8-coverage`, `c8`, etc.) e workspace link `@gluezero/core: workspace:*` → `@gluezero/mapper`
...
| `pnpm --filter @gluezero/mapper typecheck` | Exit 0 (no errori TS) |
| `pnpm --filter @gluezero/mapper test` | Exit 0 (passWithNoTests; no test files yet — atteso) |
| `pnpm --filter @gluezero/mapper build` | Exit 0; `dist/index.js` 68 B + `dist/index.d.ts` 13 B + sourcemap 69 B |
| `pnpm --filter @gluezero/core test` | Exit 0; **24 test files / 248 test passing** (no regression Phase 1) |
...
| T-02-01-02 (Repudiation — Build out-of-order mapper-prima-di-core → import error) | mitigate | `pnpm -r --filter='./packages/*' --if-present run build` rispetta dipendenze workspace; `tsup external: '@gluezero/core'` non bundla, `dist/index.js` di mapper risolve a runtime via consumer's node_modules |
...
- ✅ **Ready:** plan 02-03/04/05/06 paralleli (canonical-registry, alias-registry, transform-pipeline, valibot-adapter) possono importare da `@gluezero/core` via workspace link
- ⏳ **Pending:** plan 02-09 attiverà `import './augment'` nel barrel quando `augment.ts` esisterà
- ⏳ **Pending:** plan 02-12 estenderà CI gates root (publint/attw/size-limit) anche a `@gluezero/mapper`
