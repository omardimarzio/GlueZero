...
- **Pre-existing typecheck failure** in `@sembridge/gateway` + `@sembridge/routing build`:
  - `@sembridge/routing` non builda (out-of-scope F6 — pre-existing F3).
  - Cascade: `@sembridge/gateway` typecheck fails per missing `@sembridge/routing/dist`.
