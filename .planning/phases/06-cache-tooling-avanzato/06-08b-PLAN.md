...
must_haves:
  truths:
    ...
    - "createSemBridge(config) factory aggregato in @sembridge/sembridge implementa CHAIN COMPLETA F1+F2+F3+F4+F5+F6 (BLOCKER-2 fix esatto):
    ...
    - "createSemBridge type union completa: SemBridge = ReturnType<createBroker> | ReturnType<createMapperBroker> | ReturnType<createRouterBroker> | ReturnType<createRealtimeBroker> | ReturnType<createWorkerBroker> | ReturnType<createCacheBroker> | ReturnType<createDevtoolsBroker> (NON solo Cache | Devtools — chain completa F1..F6)"
    - "Verifica acceptance createSemBridge: `grep -E 'createWorkerBroker|createRealtimeBroker' packages/sembridge/src/sem-bridge.ts` deve trovare ENTRAMBI"
    - "Integration test 'createSemBridge default features chain F1+F2+F3+F4+F5+F6 active': verifica che pubblicare un evento attiva mapper + routing + (se features.cache=true) cache + devtools tap"
    - "Integration test 'createSemBridge { features: { realtime: false } } skip realtime': verifica che createRealtimeBroker NON è chiamato + chain procede senza step realtime"
    ...
    - "Barrel @sembridge/sembridge index.ts esporta createSemBridge + tipi + re-export pubblici sub-package"
  ...
  artifacts:
    ...
    - path: "packages/sembridge/src/sem-bridge.ts"
      provides: "createSemBridge(config) factory aggregato CHAIN COMPLETA F1+F2+F3+F4+F5+F6 (BLOCKER-2 fix) + features opt-out + D-30 + type union completa"
    ...
    - path: "packages/sembridge/src/__integration__/chain-completa-flow.test.ts"
      provides: "Tier-1 integration test createSemBridge default chain F1+F2+F3+F4+F5+F6 active end-to-end"
    - path: "packages/sembridge/src/__integration__/features-opt-out.test.ts"
...
3. **createSemBridge aggregato CHAIN COMPLETA F1+F2+F3+F4+F5+F6** (`packages/sembridge/src/sem-bridge.ts`) — **BLOCKER-2 fix critico**:
...
06-08a CacheBroker (chained inside createSemBridge if features.cache):
...
| T-06-08b-04 | Tampering (config.features bypass — consumer fa createSemBridge({features: {cache: false}}) ma cacheRoutes presenti) | sem-bridge | mitigate | Validation feature consistency: se cacheRoutes definite ma features.cache===false → throw "Invalid SemBridgeConfig: cache routes defined but feature disabled". Test |
| T-06-08b-05 | Logic flaw (chain completa NON include realtime/worker — BLOCKER-2 regression) | sem-bridge | mitigate | Acceptance grep `createWorkerBroker|createRealtimeBroker` in sem-bridge.ts deve trovare ENTRAMBI. Integration test chain-completa verifica scenario realtime SSE inbound + worker offload entrambi attivi |
...
```
  ...
  <verify>
    <automated>cd "/Users/omarmarzio/programming/prova AI/SemBridge" && pnpm -F @sembridge/devtools test --run devtools-broker 2>&1 | tail -15 && pnpm -F @sembridge/devtools test --run public-factory 2>&1 | tail -10</automated>
  </verify>
  <done>
...
<task type="auto" tdd="true">
  <name>Task 2: sem-bridge.ts (createSemBridge CHAIN COMPLETA F1+F2+F3+F4+F5+F6 — BLOCKER-2 fix)</name>
  <files>packages/sembridge/src/sem-bridge.ts, packages/sembridge/src/sem-bridge.test.ts</files>
  <read_first>
  ...
  <behavior>
    - Test 1: createSemBridge({}) ritorna SemBridge con CHAIN COMPLETA F1+F2+F3+F4+F5+F6 attiva (default features tutte enabled)
    - Test 2: createSemBridge({ features: { realtime: false } }) → createRealtimeBroker NON chiamato + chain procede senza step realtime
    - Test 3: createSemBridge({ features: { worker: false } }) → createWorkerBroker NON chiamato
    - Test 4: createSemBridge({ features: { cache: false } }) → createCacheBroker NON chiamato
    - Test 5: createSemBridge({ features: { devtools: false } }) → createDevtoolsBroker NON chiamato
    - Test 6: createSemBridge({ features: { realtime: false, worker: false } }) → solo F1+F2+F3+F6 chain
    - Test 7: D-30 multi-tenant isolation — 2 createSemBridge istanze indipendenti (NO singleton)
    - Test 8: Valibot fail prefix verified — config.features={cache:'string'} → throw 'Invalid SemBridgeConfig:'
    - Test 9: type union completa — `SemBridge` accetta ReturnType di TUTTI i 7 factory (compile-time check)
    - Test 10: Acceptance grep — sem-bridge.ts source contiene riferimento sia a `createWorkerBroker` che `createRealtimeBroker` (BLOCKER-2 fix verification)
  </behavior>
  <action>
RED + GREEN TDD pattern.

GREEN sem-bridge.ts (~120 LOC) — implementazione **CHAIN COMPLETA F1+F2+F3+F4+F5+F6 (BLOCKER-2 fix esatto)**:
...
/**
 * Crea un SemBridge aggregato con chain composition COMPLETA F1+F2+F3+F4+F5+F6.
 *
...
grep -E "createWorkerBroker|createRealtimeBroker" packages/sembridge/src/sem-bridge.ts | wc -l
...
```bash
git add packages/sembridge/src/sem-bridge.ts packages/sembridge/src/sem-bridge.test.ts
git commit -m "feat(06-08b): GREEN createSemBridge CHAIN COMPLETA F1+F2+F3+F4+F5+F6 (BLOCKER-2 fix)

createSemBridge(config) factory aggregato chain composition COMPLETA — features opt-out
...
Type union SemBridge = ReturnType<createBroker | createMapperBroker | createRouterBroker |
...
```
  ...
  <verify>
    <automated>cd "/Users/omarmarzio/programming/prova AI/SemBridge" && pnpm -F @sembridge/sembridge test --run sem-bridge 2>&1 | tail -15 && grep -cE "createWorkerBroker|createRealtimeBroker" packages/sembridge/src/sem-bridge.ts</automated>
  </verify>
  <done>
    - sem-bridge.ts ~120 LOC + .test.ts 10+ test
...
<task type="auto">
  ...
  <files>packages/devtools/src/index.ts, packages/sembridge/src/index.ts, packages/devtools/src/__integration__/multiplex-tap-flow.test.ts, packages/devtools/src/__integration__/pause-resume-flow.test.ts, packages/devtools/src/__integration__/metrics-cardinality-flow.test.ts, packages/devtools/src/__integration__/inspector-snapshot.test.ts, packages/sembridge/src/__integration__/chain-completa-flow.test.ts, packages/sembridge/src/__integration__/features-opt-out.test.ts</files>
  <read_first>
    ...
    - packages/sembridge/src/sem-bridge.ts (creato Task 2)
...
**3.2 — Barrel `packages/sembridge/src/index.ts`:**
...
- `chain-completa-flow.test.ts` (3 scenari):
  - Test A: createSemBridge default → publish event → mapper attivo (canonical) + routing attivo (route execution) + cache attiva (se cache route) + devtools tap registra evento
  - Test B: createSemBridge default + cache route + worker route → entrambi attivi (cache hit rapido + worker offload async)
  - Test C: createSemBridge default + realtime SSE inbound mock → realtime adapter attivo + tap cattura `realtime.connected` lifecycle
...
```bash
git add packages/devtools/src/index.ts packages/sembridge/src/index.ts packages/devtools/src/__integration__/ packages/sembridge/src/__integration__/
...
Barrel sembridge: side-effect re-export augment + createSemBridge + re-export pubblico API
surface da @sembridge/cache + @sembridge/devtools.
...
SemBridge integration (2 file — BLOCKER-2 acceptance):
- chain-completa-flow.test.ts (3): createSemBridge default chain F1+F2+F3+F4+F5+F6 active end-to-end
...
- **BLOCKER-2 fix verified**: createSemBridge include CHAIN COMPLETA F1+F2+F3+F4+F5+F6 — acceptance grep `createWorkerBroker|createRealtimeBroker` ≥4 hits
- WARNING-4 fix applicato: verify Task 3 include `pnpm -F @sembridge/sembridge test`
...
- [x] **createSemBridge CHAIN COMPLETA F1+F2+F3+F4+F5+F6 (BLOCKER-2 fix)** ✅
