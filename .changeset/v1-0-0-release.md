...
| ------------------------ | -------- |
| `@sembridge/core`        | ~6 KB    |
| `@sembridge/mapper`      | ~12 KB   |
| `@sembridge/routing`     | ~19 KB   |
| `@sembridge/gateway`     | ~6 KB (HTTP) + sse-ws sub-modulo |
| `@sembridge/worker`      | ~26 KB   |
| `@sembridge/cache`       | ~22 KB   |
| `@sembridge/devtools`    | ~22 KB   |
| `@sembridge/sembridge`   | ~35 KB   |

## V1.x Roadmap (deferred)

- `@sembridge/cache-idb` (IndexedDB persistence)
- `@sembridge/metrics-prometheus` (Prometheus textfile exporter)
- `@sembridge/metrics-otel` (OpenTelemetry SDK adapter)
