// augment.ts — TS declaration merging per estendere @gluezero/core con `BrokerConfig.gateway`.
...
//
// Cosa estende:
//   - BrokerConfig (interface, da @gluezero/core) — aggiunge `gateway?: GatewayConfig` come
...
//
// Perché qui (separato da `@gluezero/routing`):
//   - `@gluezero/gateway` ha già `@gluezero/routing` come dependency workspace (vedi
...
//     '@gluezero/gateway'` da routing → ciclo workspace.
...
//     (T-03-04-02): consumer che importa entrambi `@gluezero/routing` e `@gluezero/gateway`
...
//     — sono già coperti da `F3PipelineStep` di `@gluezero/routing`). F4 SSE/WS aggiungerà
...
//   `BrokerConfig` da `@gluezero/routing` (campi `routes`/`routing`) e `@gluezero/gateway`
